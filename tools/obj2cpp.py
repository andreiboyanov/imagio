...
def main(argv):
    ...
    with open(os.path.join(output_folder, mesh_name + ".cpp"), "w+") as c_file:
        c_file.write("// Auto-generated file with definition of a {} mesh\n".format(mesh_name))
        c_file.write('#include "{}.h"\n'.format(mesh_name))
        c_file.write("\n")
        c_file.write("namespace imagio\n")
        c_file.write("{\n")
        c_file.write("namespace meshes\n")
        c_file.write("{\n")
        c_file.write("namespace cube\n")
        c_file.write("{\n")
        c_file.write("const int vertice_count = {};\n".format(vertice_count))
        c_file.write("const float vertices[{}] = \n".format(vertice_count * 3))
        c_file.write("{\n")
        ...
        c_file.write("};\n")
        c_file.write("const float normals[{}] = \n".format(vertice_count * 3))
        c_file.write("{\n")
        for i in range(triangle_count):
        ...
        c_file.write("};\n")
        c_file.write("const float texture_coordinates[{}] = \n".format(vertice_count * 2))
        c_file.write("{\n")
        for i in range(triangle_count):
